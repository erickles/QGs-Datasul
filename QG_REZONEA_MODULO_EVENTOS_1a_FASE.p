...
INPUT FROM "C:\Listas_Rezoneamento\lista_rezoEventosESRJ.csv" CONVERT TARGET "ibm850".
...
FOR EACH tt-planilha NO-LOCK:
    ...
    FOR EACH es-eventos WHERE TRIM(es-eventos.nome-mic-reg) = TRIM(tt-planilha.micro-antiga).
    ...
    FOR EACH es-eventos WHERE TRIM(es-eventos.nome-ab-reg) = TRIM(tt-planilha.regiao-antiga).
        ASSIGN es-eventos.nome-ab-reg   = tt-planilha.regiao-nova
               es-eventos.unidade-venda = tt-planilha.gerencia-nova.
    END.
    
    
    FOR EACH es-ev-regiao-unid WHERE TRIM(es-ev-regiao-unid.nome-ab-reg) = TRIM(tt-planilha.regiao-antiga):
    ...
    FOR EACH es-ft-gerencias WHERE TRIM(es-ft-gerencias.nome-ab-reg) = TRIM(tt-planilha.regiao-antiga).
