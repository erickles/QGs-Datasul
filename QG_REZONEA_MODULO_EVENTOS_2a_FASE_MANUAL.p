...
UPDATE cMicroAntiga 
       ...
       cRegiaoAntiga
       cRegiaoNova  
       cGerenciaNova WITH 1 COL.
   
/* Regiao nova */
FOR EACH es-eventos WHERE TRIM(es-eventos.nome-mic-reg) = TRIM(cMicroNova).
    ASSIGN es-eventos.nome-mic-reg  = cMicroNova
           es-eventos.nome-ab-reg   = cRegiaoNova
           es-eventos.unidade-venda = cGerenciaNova.
END.

FOR EACH es-eventos WHERE TRIM(es-eventos.nome-ab-reg) = TRIM(cRegiaoNova).
    ASSIGN es-eventos.nome-ab-reg   = cRegiaoNova
           es-eventos.unidade-venda = cGerenciaNova.
END.

FOR EACH es-ev-regiao-unid WHERE TRIM(es-ev-regiao-unid.nome-ab-reg) = TRIM(cRegiaoNova):
...
FOR EACH es-ft-gerencias WHERE TRIM(es-ft-gerencias.nome-ab-reg) = TRIM(cRegiaoNova).
