...
for each tt-nota-matriz no-lock break by tt-nota-matriz.i-random
    ...
    if last-of(tt-nota-matriz.i-random) then do:
        ...
        run calcula-6-meses(input  tt-nota-matriz.tt-dt-emis,
                            output data-saida).
        ...
        if avail bf-nota-matriz then tt-nota-matriz.c-status = "RECOMPRA".
        ...
        if not avail bf-nota-matriz then do:
            FIND last bf-nota-matriz WHERE bf-nota-matriz.i-random   = tt-nota-matriz.i-random
                                       ...
                                       NO-LOCK NO-ERROR.
            if avail bf-nota-matriz then tt-nota-matriz.c-status = "RECUPERADO".
        end.

        if i = 1 then tt-nota-matriz.c-status = "NOVO CLIENTE".
           ...
           tt-nota-matriz.c-status = ? then do:
            ...
            for each bf-nota-matriz where bf-nota-matriz.c-status = "" or
                ...
                i = i + 1.
                    ...
                    if avail bf2-nota-matriz then bf-nota-matriz.c-status = "RECOMPRA".
    
                    if not avail bf2-nota-matriz then assign bf-nota-matriz.c-status = "NOVO CLIENTE".
        ...
        if tt-nota-matriz.c-status = "RECOMPRA" then do:
            if (month(dt-inicial) = month(tt-nota-matriz.tt-dt-emis) and
               year(dt-inicial)  = year(tt-nota-matriz.tt-dt-emis)) then
                tt-nota-matriz.c-status = "NOVO CLIENTE".
        end.
    end.

    if tt-nota-matriz.c-status = "NOVO CLIENTE" AND (
       (tt-nota-matriz.tt-emit-m < 70000        OR
       (tt-nota-matriz.tt-emit-m >= 90000       AND
        tt-nota-matriz.tt-emit-m <= 100000))
       OR
        (tt-nota-matriz.tt-emit < 70000         OR
       (tt-nota-matriz.tt-emit >= 90000         AND
        tt-nota-matriz.tt-emit <= 100000))  )
        THEN
        tt-nota-matriz.c-status = "RECUPERADO CICS".
    
    disp trim(tt-nota-matriz.tt-nr-nota)
         tt-nota-matriz.tt-dt-emis
         tt-nota-matriz.tt-emit   
         tt-nota-matriz.tt-emit-m label "Matriz"
         tt-nota-matriz.i-random
         tt-nota-matriz.c-status format "x(12)".
