...
FOR EACH es-loc-entr WHERE es-loc-entr.boleto      = 1
                       AND es-loc-entr.cod-entrega = "Padrao" NO-LOCK,
    EACH emitente WHERE emitente.nome-abrev = es-loc-entr.nome-abrev
                    AND emitente.cod-gr-cli = 2
                    AND emitente.cod-emitente <= 1001
                    AND (emitente.identific = 1 OR emitente.identific = 3)
                    AND (emitente.portador <> 112 OR emitente.port-prefer <> 112) BREAK BY emitente.cod-emitente:            
